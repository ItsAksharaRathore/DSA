...
do
{
cout << "Enter function number: " << endl; cin >> choice;
switch (choice)
{
case 1:
insert_beg(); break;
...
{
cout << "Deleted node contains: " << head->data << endl; node *temp = head;
while (temp->next != head)
{
temp = temp->next;
}
...
cout << "Deleted node contains: " << (temp->next)->data << endl; temp->next = head;
}
}
...
{
cout << "Deleted node contains: " << head->data << endl; if (head->next == head)
{
...
cout << "Deleted node contains: " << (temp->next)->data << endl; temp->next = (temp->next)->next;
}
}
}
